...
from hookscan.detectors.all_detectors import all_detectors, all_detectors_dict
from hookscan.detectors.detector_result import DetectorResult
from hookscan.hookscan import Hookscan
from hookscan.utils.cfg_visualizer import generate_cfg
from hookscan.utils.compiler import compile_standard_json, generate_standard_input_json


class HookscanJSONEncoder(JSONEncoder):
...
def parse_cli_args() -> Namespace:
    parser = argparse.ArgumentParser(prog="hookscan", description="vulnerability detection tool for Solidity")
...
def execute_and_output(args: Namespace) -> None:
    try:
        ...
        if args.mode == "cfg":
            ...
            generate_cfg(hookscan.contract, f"{args.output}/{hookscan.contract.file_name}", render=True)
            return
        ...
        hookscan.register_detectors(detectors)
        result = hookscan.detect()
        detection_results = []
    ...
    if args.output:
        ...
        with open(args.output, "w") as of:
            json.dump(result, of, indent=4, cls=HookscanJSONEncoder)
    else:
        print(json.dumps(result, indent=4, cls=HookscanJSONEncoder))
